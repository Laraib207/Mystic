  ...
  return (
    <>
      ...
      >
          ...
          <div className="flex items-center justify-between gap-8">
            ...
            >
              ...
              <div className="relative w-14 h-14 sm:w-16 sm:h-16 flex-shrink-0">
                <div className="absolute inset-0 rounded-full bg-[#C9A24D]/25 blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                <div className="relative w-full h-full rounded-full bg-gradient-to-br from-[#C9A24D]/15 to-transparent p-0.5">
                  <Image
                    src="/logo/logo.png"
                    alt="Mystic Moments"
                    width={64}
              ...
              {/* Logo Text */}
              <div className="hidden sm:block">
                <p className="text-[9px] tracking-[0.3em] uppercase text-white/60 leading-tight">
                  Luxury Event Curators
                </p>
                <p className="text-xl md:text-2xl font-serif text-white leading-tight mt-0.5">
                  Mystic{" "}
                  <span className="text-[#C9A24D] drop-shadow-[0_0_12px_rgba(201,162,77,0.8)]">
                    Moments
                  </span>
                </p>
              </div>
            ...
            <div className="hidden lg:flex items-center gap-8">
              <ul className="flex items-center gap-7 xl:gap-8 text-[12px] font-medium">
                {navLinks.map((link) => (
                  <li key={link.href} className="relative">
                    <Link
                      href={link.href}
                      className="relative text-white/80 hover:text-white tracking-[0.12em] uppercase transition-colors duration-300 group"
                    >
                      {link.name}
                      <span className="pointer-events-none absolute -bottom-1 left-0 h-[1px] w-0 bg-gradient-to-r from-[#C9A24D] via-amber-300 to-white group-hover:w-full transition-all duration-300" />
                    </Link>
                  </li>
                ))}
              </ul>
              ...
              {/* DESKTOP CTA */}
              <button className="group relative inline-flex items-center gap-2 rounded-full border border-[#C9A24D]/70 bg-[#C9A24D] px-6 py-2.5 text-[11px] font-bold tracking-[0.16em] text-black shadow-[0_0_24px_rgba(201,162,77,0.7)] transition-all duration-300 hover:bg-[#E5C87A] hover:shadow-[0_0_32px_rgba(201,162,77,0.9)] hover:scale-105">
                <span>BOOK EVENT</span>
                <span className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-black/15 text-[9px]">
                  →
                </span>
                <span className="absolute inset-0 -z-10 rounded-full bg-gradient-to-r from-[#C9A24D] via-white/30 to-[#C9A24D] opacity-0 blur-lg group-hover:opacity-100 transition-opacity" />
              </button>
            </div>
        ...
        {/* Smooth Marquee Text Bar */}
        <div className="relative bg-gradient-to-r from-black/40 via-[#C9A24D]/8 to-black/40 border-t border-[#C9A24D]/15 overflow-hidden py-3">
          {/* Marquee Container */}
          <div className="flex items-center">
            <div className="animate-marquee flex items-center gap-12 whitespace-nowrap">
              {/* Duplicate texts twice for seamless loop */}
              {[...Array(2)].map((_, setIndex) => (
                <div key={setIndex} className="flex items-center gap-12">
                  {scrollingTexts.map((text, index) => (
                    <span
                      key={`${setIndex}-${index}`}
                      className="text-2xl sm:text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white/90 via-[#C9A24D] to-white/90 inline-block"
                    >
                      {text}
                    </span>
                  ))}
          ...
          {/* Gradient Fade Edges */}
          <div className="absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-black/90 to-transparent pointer-events-none" />
          <div className="absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-black/90 to-transparent pointer-events-none" />
        </div>
      </header>
      ...
      >
        <div
          className={`absolute inset-0 bg-gradient-to-br from-black/96 via-slate-900/96 to-black/96 backdrop-blur-2xl transition-opacity duration-500 ${
            open ? "opacity-100" : "opacity-0"
          }`}
          onClick={() => setOpen(false)}
        />
        ...
        >
            ...
            {navLinks.map((link, i) => (
              ...
              >
                <Link
                  href={link.href}
                  onClick={() => setOpen(false)}
                  className="text-2xl sm:text-3xl font-light tracking-[0.18em] uppercase text-white hover:text-[#C9A24D] transition-colors duration-300"
                >
                  {link.name}
                </Link>
              </li>
          ...
          <button
            onClick={() => setOpen(false)}
            className="group relative inline-flex items-center gap-2 rounded-full border border-[#C9A24D]/70 bg-[#C9A24D] px-10 py-4 text-[13px] font-bold tracking-[0.18em] text-black shadow-[0_0_30px_rgba(201,162,77,0.8)] transition-all duration-300 hover:bg-[#E5C87A] hover:scale-105 mt-4"
          >
            <span>BOOK EVENT</span>
            <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-black/15 text-[10px]">
              →
            </span>
            <span className="absolute inset-0 -z-10 rounded-full bg-[#C9A24D]/40 blur-xl opacity-0 group-hover:opacity-100 transition-opacity" />
          </button>

          {/* Decorative glows */}
          <div className="pointer-events-none absolute top-20 left-10 h-32 w-32 rounded-full bg-[#C9A24D]/20 blur-3xl" />
          <div className="pointer-events-none absolute bottom-20 right-10 h-32 w-32 rounded-full bg-purple-500/20 blur-3xl" />
        </div>
      </div>
